public class FeuPokemon extends Pokemon {
    ...
    public FeuPokemon(int numeroNational, String nom, int niveau,
                      int pointsDeVie, int attaque, int defense,
                      int attaqueSpeciale, int defenseSpeciale, int vitesse) {
        super(numeroNational, nom, "Feu", niveau, pointsDeVie, attaque, defense,
                attaqueSpeciale, defenseSpeciale, vitesse);
    }

    @Override
    public void attaquer(Pokemon adversaire) {
        System.out.println(getNom() + " utilise 火焰喷射 !");
        int degats = calculerDommage(adversaire, 40, true); // 火花，威力40，特殊攻击
